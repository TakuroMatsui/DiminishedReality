...
class Detector:
    ...
    def _readData(self,path,name):
        ...
        outputData=cv2.imread(path+'output/'+name,0)/255.0
        outputData=np.reshape(outputData,[-1,self.size,self.size,1])[0]

        return [inputData],[outputData]

    def _showImages(self,real,out):
        items=len(real)
        images=np.zeros([self.size*items,self.size*2,3])

        _out=np.zeros([items,self.size,self.size,3])
        for k in range(items):
            for i in range(self.size):
                for j in range(self.size):
                    _out[k,i,j]=[out[k,i,j],out[k,i,j],out[k,i,j]]

        for i in range(items):
            images[self.size*i:self.size*(i+1),0:self.size,:]=real[i,:,:,0:3]
            images[self.size*i:self.size*(i+1),self.size:self.size*2,:]=_out[i,:,:]
        images=cv2.resize(images,(256,128*5))
        cv2.imshow("real : out",images)
        cv2.waitKey(1)
    ...
    def _buildGenerator(self,x,keep_prob,reuse):
        with tf.variable_scope("Generator") as scope:
            ...
            w,b=self._conv_variable([self.Filter,self.Filter,self.Layer,1],"deconv3-out")
            h=self._conv2d(h,w,1)+b
    ...
    def _buidModel(self):
        with self.graph.as_default():
            e=0.00000001
            self.gx=tf.placeholder(tf.float32,[None,self.size,self.size,3],name="gx")
            self.gy_=tf.placeholder(tf.float32,[None,self.size,self.size,1],name="gy_")
